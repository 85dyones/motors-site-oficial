...
export default function InvestidoresPainel() {
  ...
  const handleInvestidorSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const url = editandoId
        ? `/api/financeiro/investidores/${editandoId}`
        : "/api/financeiro/investidores";
  ...
  const handleMovimentacaoSubmit = async (e: React.FormEvent) => {
    ...
    setError("");
    try {
      const res = await fetch("/api/financeiro/investidores/movimentacoes", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
  ...
  const handleExcluirMovimentacao = async (m: Movimentacao) => {
    ...
    if (!ok) return;
    try {
      const res = await fetch(`/api/financeiro/investidores/movimentacoes/${m.id}`, {
        method: "DELETE",
      });
      const body = await res.json();
      if (res.ok) {
