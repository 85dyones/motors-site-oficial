---
url: /guias/cautelar-x-vistoria-transferencia
pilar: 1 — Procedência e perícia cautelar
papel: SPOKE — comparação, formato tabela
onda: 1
keyword_primaria: laudo cautelar ou vistoria de transferência
keyword_secundarias: diferença vistoria cautelar e transferência · vistoria do detran verifica sinistro · vistoria de transferência o que é checado
saida_comercial: /estoque
palavras: ~1.700
---

# Laudo cautelar x vistoria de transferência

**Title:** `Laudo cautelar x vistoria de transferência: a diferença`
**Meta description:** `A vistoria obrigatória do Detran não atesta procedência. Ela confere identificação e equipamentos. O que cada exame verifica, e por que a ordem importa.`
...
| | Vistoria de transferência | Laudo cautelar |
|---|---|---|
| **Obrigatória?** | Sim, para transferir | Não |
| **Para quem** | Para o órgão de trânsito | Para você |
| **Quem faz** | Detran ou ECV credenciada | Empresa privada especializada |
| **Identificação** | Sim | Sim |
| **Equipamentos obrigatórios** | Sim | Não |
| **Integridade estrutural** | **Não** | Sim |
| **Espessura de pintura** | Não | Sim |
| **Histórico de leilão** | **Não** | Sim, via módulo de pesquisa |
| **Registro de sinistro** | Não como avaliação | Sim |
| **Quando acontece** | Depois de você comprar | **Antes de você decidir** |
| **Resultado** | Libera ou não o processo | Documento com o que foi encontrado |
...
Repare no que **não** está no escopo:

- integridade estrutural — longarinas, colunas, assoalho, torres de suspensão
- teste de espessura de pintura para identificar reparo
- histórico de passagem por leilão
- avaliação do registro de sinistro e da gravidade do dano
- estado mecânico
...
A diferença está na profundidade e na abrangência. O laudo cautelar confere vários pontos de identificação — etiquetas de carroceria, gravação em vidros, numeração de câmbio — e cruza tudo com o registro, além de somar estrutura e histórico ao quadro. [A conferência cruzada é o que expõe adulteração bem feita](/guias/chassi-remarcado).
...
Onde fazer perícia cautelar na cidade, com prazos e faixas de preço: [guia de Curitiba](/guias/pericia-cautelar-curitiba).
...
**Nos nossos carros o cautelar já foi feito antes do anúncio, e o resultado está publicado na ficha para você ler antes de decidir.** O que sobra é a parte burocrática — e nela a gente ajuda.
...
Pode pegar, porque confere numeração de chassi e motor. Adulteração bem executada, no entanto, costuma só aparecer no cruzamento de vários pontos de identificação — que é o que o [cautelar faz](/guias/laudo-cautelar-carro-usado).
...
| "histórico de passagem por leilão" (o que não verifica) | `/guias/consultar-carro-leilao-sinistro` |
| "gravidade do dano" | `/guias/resultados-laudo-cautelar` |
| "guia de Curitiba" | `/guias/pericia-cautelar-curitiba` |
| "cautelar faz" (FAQ) | `/guias/laudo-cautelar-carro-usado` **(pilar)** |
| CTA final | `/estoque` |

**Entradas:** pilar · `/guias/pericia-cautelar-curitiba` · hub · FAQ global fase 2

---

## Schema

`Article` + `BreadcrumbList` + `FAQPage`. `about`: Vistoria veicular · Laudo cautelar · Transferência de propriedade.
...
- [x] T3 — coerente com o pilar e com os demais spokes
- [x] T4 — descritivo. Nenhuma ECV ou empresa nomeada. Não desqualifica a vistoria obrigatória; delimita o escopo dela
- [x] T5 — cita fonte oficial do Detran-PR e encaminha ao portal para confirmação
- [x] T6 — nenhum número sem origem. O prazo é atribuído ao Detran-PR
- [x] Critério 3 — uma saída comercial
- [x] H1 é a query, 42 caracteres
- [x] Primeiro parágrafo responde em duas frases
- [x] Tabela comparativa, no topo — é peça de comparação
