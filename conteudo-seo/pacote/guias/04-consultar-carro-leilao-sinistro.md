---
url: /guias/consultar-carro-leilao-sinistro
pilar: 1 — Procedência e perícia cautelar
papel: SPOKE — maior volume de busca da onda
onda: 1
keyword_primaria: como saber se o carro é de leilão
keyword_secundarias: consultar se carro tem sinistro · carro de leilão consta no documento · carro batido recuperado como identificar
saida_comercial: /estoque
palavras: ~2.000
---

# Como saber se um carro passou por leilão

**Title:** `Como saber se um carro passou por leilão | Motors Store`
**Meta description:** `Passagem por leilão não consta no documento do veículo. Onde essa informação aparece, o que a consulta de placa mostra e por que leilão não é sinônimo de sinistro.`
...
O que a base oficial **registra**, e vale consultar de qualquer forma:

- **Sinistro de média monta** — observação "Sinistro/Recuperado", permanente
- **Baixa por grande monta** — o veículo não pode circular nem ser transferido
- **Restrições** administrativas e judiciais
- **Gravame** de financiamento ativo
- **Roubo e furto**
...
> **Verificação de T3 antes de publicar.** A Motors Store compra lotes de desmobilização de frota. Se algum desses lotes chega por canal de leilão, o texto não pode afirmar que passagem por leilão é critério de recusa absoluto. A distinção acima resolve — mas confirme a redação do bloco "o que fazemos com isso" contra a prática real de compra.
...
Nenhum desses sinais isolado prova nada. Juntos, e cruzados com o histórico, formam um quadro — e é exatamente esse cruzamento que uma [perícia cautelar](/guias/laudo-cautelar-carro-usado) faz de forma sistemática.
...
**Cheque o documento por sinistro.** Se houver observação de recuperado, a origem provavelmente foi seguradora e o dano foi estrutural. Isso [muda o patamar do negócio](/guias/resultados-laudo-cautelar).
...
**No nosso estoque as duas já foram feitas, e o resultado publicado na ficha cobre as duas — inspeção física e pesquisa de histórico.**
...
Pode, desde que esteja regularizado. A exceção é o veículo com baixa por grande monta, que não pode voltar a circular, licenciar ou ser transferido.

**Dá para consultar passagem por leilão de graça?**
As bases gratuitas mostram restrições, gravame e ocorrência de roubo ou furto, mas não histórico de leilão. Para isso é preciso base privada de histórico veicular ou o módulo de pesquisa da perícia cautelar. [Onde fazer em Curitiba](/guias/pericia-cautelar-curitiba).

**É possível apagar o histórico de leilão?**
Não. A informação fica em bases de dados que não são controladas pelo proprietário. Qualquer oferta de "limpar" o histórico deve ser tratada como o que é.

**Carro de leilão dá para financiar e segurar?**
Em geral sim, se estiver regularizado, mas as condições costumam ser piores e alguns bancos e seguradoras recusam. Vale confirmar antes de fechar, não depois.
...
| Trecho | Destino |
|---|---|
| "perícia cautelar" (sinais físicos) | `/guias/laudo-cautelar-carro-usado` **(pilar)** |
| "muda o patamar do negócio" | `/guias/resultados-laudo-cautelar` |
| "Etiquetas de identificação ausentes ou trocadas" | `/guias/chassi-remarcado` |
| "três entram" | `/guias/o-que-reprova-pericia-cautelar` |
| "Onde fazer em Curitiba" (FAQ) | `/guias/pericia-cautelar-curitiba` |
| CTA final | `/estoque` |

**Entradas:** pilar · `/guias/resultados-laudo-cautelar` · `/guias/pericia-cautelar-curitiba` · hub · FAQ global fase 2

---

## Schema

`Article` + `BreadcrumbList` + `FAQPage`. `about`: Leilão de veículos · Histórico veicular · Sinistro.
...
- [x] T1 e T2 — sem multiplicadores. "Desvaloriza" aparece sem percentual e sem referência a FIPE
- [x] T3 — **marcado para verificação**, ver acima
- [x] T4 — descritivo. Nenhuma leiloeira, base de dados ou concorrente nomeado
- [x] T5 — cita o art. 22 do CTB como delimitação de competência, sem parecer jurídico
- [x] T6 — o único número é o 3 em 10, dado próprio
- [x] Critério 3 — uma saída comercial
- [x] H1 é a query, 39 caracteres
