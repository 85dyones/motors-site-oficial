---
url: /guias/motores-turbo-usados-o-que-checar
pilar: 3 — Mecânica de risco do seminovo moderno
papel: PILAR
onda: 2
keyword_primaria: motor turbo usado o que checar
keyword_secundarias: motor 1.0 turbo usado vale a pena · motor três cilindros turbo problemas · como saber se o turbo está ruim · downsizing em carro usado
saida_comercial: /garantia
palavras: ~2.700
onda2_total: 3 peças nesta entrega
---

# Motor turbo de baixa cilindrada usado: o que checar

**Title:** `Motor turbo de baixa cilindrada usado: o que checar`
**Meta description:** `Óleo fora de lugar, folga no eixo, fumaça na retomada e histórico de óleo com nota. O que checar num 1.0 ou 1.3 turbo usado — e o que só aparece rodando.`
...
O plano de garantia estendida vendido à parte não cobre turbocompressor — isso está escrito nele. A garantia da loja, de três meses a partir da entrega, cobre falha interna de motor, câmbio e diferencial, com mão de obra inclusa, e exige que você avise a loja antes de levar o carro a qualquer oficina. O que entra e o que não entra, item a item, está na página de garantia.

---

## Links internos

Nenhum link escrito no corpo: o renderizador serve parágrafo como texto puro e quem cria link é `TERMOS_COM_DESTINO` (`src/lib/linksNoTexto.ts`). As citações abaixo estão pelo título, prontas para virar âncora quando os termos forem cadastrados.

| Trecho | Destino |
|---|---|
| "Laudo cautelar: o que verifica e o que não verifica" (2 citações) | `/guias/laudo-cautelar-carro-usado` **(pilar da Onda 1)** |
| "O que reprova um carro na perícia cautelar" | `/guias/o-que-reprova-pericia-cautelar` |
| "Correia dentada banhada em óleo: por que ela falha" | `/guias/correia-dentada-banhada-em-oleo` **(Onda 2)** |
| "Carbonização de válvulas em injeção direta" | `/guias/carbonizacao-valvulas-injecao-direta` **(Onda 2)** |
| CTA final | `/garantia` |

**Entradas:** os dois spokes desta onda · hub `/guias` · `/garantia` · ficha de veículo com motor turbo (R8)

**A cadastrar em `TERMOS_COM_DESTINO` quando as três peças estiverem publicadas:** os três títulos da Onda 2 e, se o dono quiser o assunto e não só o título, "motor turbo de baixa cilindrada".

---

## Schema

`Article` + `BreadcrumbList` + `FAQPage`. Autor e publisher no nó `#dealer`.

```json
{
  "@context": "https://schema.org",
  "@type": "Article",
  "@id": "https://motorsstore.com.br/guias/motores-turbo-usados-o-que-checar#article",
  "headline": "Motor turbo de baixa cilindrada usado: o que checar",
  "description": "O que checar num motor turbo de baixa cilindrada usado: óleo fora de lugar, folga no eixo, fumaça na retomada, consumo de óleo e histórico de manutenção.",
  "inLanguage": "pt-BR",
  ...
  "mainEntityOfPage": "https://motorsstore.com.br/guias/motores-turbo-usados-o-que-checar",
  "about": [
    { "@type": "Thing", "name": "Motor turbo de baixa cilindrada" },
    { "@type": "Thing", "name": "Turbocompressor" },
    { "@type": "Thing", "name": "Compra de carro seminovo" }
  ]
}
```
...
- **A garantia de três meses cobre turbocompressor?** O texto não afirma nem nega: os fatos confirmados dizem que ela cobre falha interna de motor, câmbio e diferencial, e o turbo não está nem na lista de cobertura nem na de exclusões. É pergunta para o dono. Enquanto não houver resposta, a peça encaminha para `/garantia`.
- **Limite de quilometragem da garantia da loja:** existe, valor não informado. Nenhum número de km aparece aqui.
- **Nomear famílias de motor** (fabricante e código) tornaria a peça mais útil e mais arriscada, porque a loja vende esses carros. Decisão editorial tomada: descrever a tecnologia, não a marca. Reversível com aval do dono.
...
- [x] T2 — nenhuma ocorrência de "abaixo da FIPE" ou "desconto" como argumento de compra
- [x] T3 — nada que contradiga `/garantia` nem prática da casa. A perícia mecânica é descrita como sob demanda, que é o que ela é
- [x] T4 — descritivo. Nenhum fabricante acusado, nenhum concorrente nomeado
- [x] T5 — sem parecer jurídico
- [x] T6 — os únicos números são fatos da casa: três meses de garantia, 7.000 km ou 6 meses do plano, mais de quinze oficinas, 120 pontos da cautelar, três em dez. Nenhuma estatística, nenhum percentual, nenhum preço
- [x] Regra 1 dos fatos — laudo fica com o vendedor e sai a pedido. Nunca "na ficha"
- [x] Regra 2 dos fatos — nenhum ranking de motivo de reprovação sem amostra
- [x] Regra 4 dos fatos — sem premium, luxo, exclusivo, consulte-nos, melhores preços ou procedência garantida
- [x] Regra 7 dos fatos — corpo em texto puro: sem tabela, sem lista, sem negrito
- [x] Critério 3 — uma saída comercial, `/garantia`, no fim, uma vez
- [x] H1 é a query, 51 caracteres
- [x] Primeiro parágrafo responde em duas frases
- [ ] Tabela comparativa — **não aplicável.** A anatomia do guia normativo (1.6) pede tabela quando há comparação; a regra 7 dos fatos, mais nova, proíbe tabela no corpo. Prevalece a regra 7, e a comparação virou parágrafo
