...
| `20260819140000_completude_da_venda.sql` | Pacote 2: completude do registro da venda (§3.2 e §9, meta >= 80%). `veiculos_vendidos.vendedor_id` passa a apontar para `profiles` (o texto livre `vendedor` fica para a base histórica), `profiles` ganha `telefone_e164`, e entram `vw_vendas_incompletas` (a rotina noturna lê daqui) e `completude_por_vendedor()`. ⏳ **Ainda não aplicada.** |
