...
| `20260822150000_aprovacao_de_recorrente.sql` | O cadastro de despesa recorrente passa pelo Gestor — o flanco que `20260821150000` deixou aberto. `aprovacao_status` é coluna SEPARADA de `ativa`: desligada pela operação e esperando aprovação são estados diferentes, e sobrepô-los faria "reativar" virar "aprovar". A geração exige as duas (`ativa and aprovada`). Default `aprovada` de propósito: toda recorrente existente está rodando hoje, e nascer `aguardando` faria a loja parar de pagar aluguel e energia. Carimbo da decisão por trigger, gêmeo do de `contas`. ✅ **Aplicada** (o "ainda não aplicada" que esta linha carregou até 2026-08-28 estava defasado — o livro-razão registra a versão). Objetos derrubados pela aposentadoria de 2026-08-28. |
...
| `20260831140000_atendimento_chatwoot_legivel_pelo_painel.sql` | O card do kanban passa a abrir a conversa no Chatwoot em vez do `wa.me` (decisão do dono, 31/08: *"vamos passar a guardar o id da conversa"*). Migração pequena porque **`atendimentos` já existia** — veio do bootstrap, com `lead_id` + `chatwoot_conversation_id` + UNIQUE, e nunca foi ligada (zero linhas). Entram só: policy de SELECT para staff (a tabela tinha RLS ligada e **zero policies**), índice em `lead_id` e FK para `leads` com `on delete set null` (eliminação a pedido do titular solta o vínculo e preserva o registro). INSERT/UPDATE seguem sem policy: quem escreve é o n8n com a chave de serviço. **Aplicada em produção em 2026-08-31.** |
| `20260831150000_atendimento_o_grant_que_faltava.sql` | 🔴 **Emenda à anterior, no mesmo dia, e a lição vale mais que a linha.** A `140000` criou a policy e o aceite conferiu que ela existia, era de SELECT e era para `authenticated` — tudo verdade, e o painel mesmo assim batia em `permission denied (42501)`: **o Postgres checa o GRANT antes da RLS**, e `atendimentos` só tinha privilégio para `service_role`. Policy sobre tabela sem grant é correta e inútil ao mesmo tempo. É `verificar-migracao-nao-so-a-funcao-esperada` aplicada a quem escreveu a régua. O aceite desta **lê a tabela como usuário de verdade** (`set local role authenticated` + `request.jwt.claims`) e exige os três contrastes: staff lê 1, não-staff lê 0, `anon` nem alcança. **Aplicada em produção em 2026-08-31.** |
| `20260910120000_erros_do_site.sql` | **A fila de triagem da natureza `quebra`** (observabilidade §12, Revisão 2 — o destino deixou de ser o Sentry e passou a ser em casa; é a **única** migração do pacote e precisa estar gravada **antes** do deploy do código). `erros` com `org_id`/`org_padrao()` + RLS, vocabulário fechado em `text` + CHECK e nunca enum (`ALTER TYPE` é vedado na janela de convivência), `natureza` sem `parada` de propósito — parada de negócio vai para o WhatsApp, não vira linha. `hash_agrupamento` é calculado na aplicação (FNV-1a sobre a mensagem normalizada); o CHECK guarda só a FORMA hex, porque duplicar a normalização em trigger criaria duas verdades. Quem grava é **sempre** a chave de serviço — daí **nenhuma policy de INSERT nem de DELETE**. O staff lê e só marca resolvido, com **`grant update` POR COLUNA** (`resolvido_em`, `resolvido_por`): policy de RLS não restringe coluna, e com grant de tabela o painel reescreveria a `mensagem` — apagaria a prova do defeito pela tela que existe para exibi-lo. Entra também `limpar_erros_antigos(dias default 90)`, SECURITY DEFINER só da `service_role` (n8n), que **recusa `dias < 7`**: com 0 seria um TRUNCATE disfarçado de rotina, e a retenção de 90 dias é a segunda camada da promessa de PII (`mensagem`/`stack` podem citar valor — a higienização é da aplicação, o banco não garante, e o `comment on table` avisa). Aceite prova por efeito e sob os papéis de verdade (`set local role` + claim), inclusive a sonda de retenção dentro de um `begin … exception` desfeito de propósito (plpgsql não tem SAVEPOINT; a variável sobrevive ao rollback do bloco) para não apagar erro real numa reaplicação. **12 mutações contra o aceite: 11 vermelhas.** ✅ **Aplicada em produção em 2026-09-10**, com OK explícito do dono. Conferida depois pelo EFEITO, e não pelo nome: RLS ligada com 2 policies e ZERO para `anon`/`public`; `has_table_privilege` do `anon` em SELECT falso; do `authenticated` em INSERT e DELETE falso e em SELECT verdadeiro; `has_column_privilege` do `authenticated` falso em `mensagem` e verdadeiro em `resolvido_em` (o grant por coluna de fato segura); `limpar_erros_antigos` inexecutável por `authenticated`; versão no livro-razão; e uma linha de sonda gravada pela chave de serviço com `org_id` vindo do default, depois removida. |
| `20260912120000_retencao_de_erros_agendada.sql` | O relógio que faltava para a promessa da `/privacidade`. A página afirma que registro técnico de erro "fica guardado por 90 dias e depois é apagado por rotina automática"; a rotina existe (`limpar_erros_antigos`, da `20260910120000`) e **não tinha chamador nenhum** — `cron.job` tinha dois jobs, `conformidade-diaria` e `abertura-de-janelas`, e nenhum era retenção. Política de privacidade sem executor é a política mentindo, e a mentira só apareceria no dia 91. Agenda `retencao-de-erros` em `0 6 * * *` UTC = **03h00 de Curitiba**: 3 h depois do último job da noite (`0 3`), fora da faixa `0 9` que a `20260819130000` reservou, e longe do expediente — o pior instante para apagar erro é aquele em que a loja está gerando erro. Idempotente pelo **upsert** de `cron.schedule` (nome existente ATUALIZA a linha), e **sem** `cron.unschedule` antes, que levantaria erro quando o job não existisse e trocaria o `jobid` a cada aplicação. **Nenhum grant foi afrouxado**: o job roda como `postgres` (é o que `cron.job.username` registra), que é o **dono** da função `security definer` — o `revoke ... from public, anon, authenticated` da `20260910120000` nunca tocou a concessão do dono. A função não muda: o piso de `dias < 7` fica, e com gatilho diário no ar ele importa mais, não menos. A autoconferência executa o comando agendado **palavra por palavra sob o papel do próprio job** e mede o efeito em duas sondas — a de 100 dias sai, a de 89 fica (89 e não 1 porque linha de um dia sobreviveria a qualquer janela que o piso permite, ficando verde sem provar nada) —, desfeitas por `raise exception` dentro do bloco; `public.erros` entra e sai com 0 linhas. ✅ **Aplicada em produção em 2026-09-15**, com o OK do dono de 2026-09-13 e depois do merge do #68. Antes, ensaiada contra a produção em 2026-09-12 (BEGIN…ROLLBACK, aceite verde, arquivo aplicado duas vezes na mesma transação sem duplicar job, e 6 mutações reprovadas) e de novo em 2026-09-15, com o aceite verde. |
